...
describe('rafflesService', () => {
  ...
  it('should get today raffles', async () => {
    const queryResult = [
      {
        id: 2066,
        ...
        raffleEndDate: '2024-04-08T05:59:59.000Z',
      },
    ];
    ...
    const fakeRaffles = queryResult.map((todayRaffle) => ({
    ...
    expect(raffles).toEqual(fakeRaffles);
    expect(raffleRepositoryMock.createQueryBuilder).toHaveBeenCalledTimes(1);
  ...
  //   await expect(rafflesservice.scrapInfo(productIdArr)).rejects.toEqual(
  //     mockError,
  //   );
  // });
});
